...
int sql_query_c::execute( const char *sql )
{
   if ( sql_connection && odb && !res )
   {
	   if ( try_execute( sql ) )
...
//__int64 sql_query_c::insert_id() // for linux
...
const char *sql_query_c::fetch_fieldname()	// for linux
{
   MYSQL_FIELD *field = 0;
...
//__int64 sql_result_c::n_rows()  // for linux
